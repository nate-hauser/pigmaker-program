...
def extract_data(input_document):
    pages = convert_from_path(r"C:\Users\jakeh\Downloads"'\\' + input_document + ".pdf",
...
def general_clean(df1):
    ...
    df1.drop(df1[df1[1] == ''].index, inplace=True)
    df1.reset_index(drop=True, inplace=True)
...
def fill_table(df1):
    ...
    #         for i in range (0,len(df1)):
    #             if pd.isna(df1.at[i,"Breeder"+str(count)]) and pd.notna(df1.at[i,"Heat Checker"+str(count)]):
    #                 df1.at[i,"Breeder"+str(count)]=df1.at[i,"Heat Checker"+str(count)]
...
def farrowing_clean(df1):
    ...
    df1.replace({"Parity": dict, "#Still": dict, "#Mum": dict, "#Live": dict, "#Weaned": dict}, inplace=True)

    dates_list = ["Date Farrowed", "Date Weaned"]
    df1[dates_list] += "23"
    df1 = convert_to_date(df1, dates_list)

    numeric_list = ["Crate#", "Parity", "#Live", "#Still", "#Mum", "#Weaned"]
    df1 = convert_to_numeric(df1, numeric_list)
...
def breeding_clean(df1):
    df1 = general_clean(df1)
    df1.replace({'AL': 'AC', 'BL': 'BV', 'PV': 'BV', 'B': 'BV', 'A': 'AC', 'H': 'HR'}, inplace=True)
